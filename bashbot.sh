...
send_markdown_message_special() {
res=$(curl -s "$MSG_URL" -d "chat_id=$1" -d "text=$(urlencode "$2")" -d "parse_mode=markdown" -d "reply_markup=$inline_keyboard")
}

edit_message() {
curl -s "$EDIT_URL" -d "chat_id=$1" -d "message_id=$2" -d "text=$3" -d "parse_mode=markdown"
}
...
forward() {
	[ "$3" = "" ] && return
	curl -s "$FORWARD_URL" -d "chat_id=$1" -d "from_chat_id=$2" -d "message_id=$3"
}
...
process_client() {
  ...
  echo $res > res.txt
	REPLYID=$(cat res.txt | grep -oe 'reply_to_message","from","id"'.* | cut -d " " -f 2)
	REPLYFIRST_NAME=$(echo "$res" | grep -oe 'reply_to_message","from","first_name"'.* | cut -d "]" -f 2 | tr -d '"')
	REPLYLAST_NAME=$(echo "$res" | grep -oe 'reply_to_message","from","last_name"'.* | cut -d " " -f 7 | tr -d '"')
	REPLYUSERNAME=$(echo "$res" | grep -oe 'reply_to_message","from","username"'.* | cut -d '"' -f 7 | tr -d '"')
...
while [ "$1" == "startbot" ]; do {
	...
	res=$(curl -s "${UPD_URL}${OFFSET}?limit=20" | ./JSON.sh/JSON.sh -s)
